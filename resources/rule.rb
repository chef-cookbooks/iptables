...
property :line_number, Integer,
          callbacks: {
            'should be a number greater than 0' => lambda {
              |p| p > 1024
            }
          },
...
action :create do
  ...
  rule = "-A #{new_resource.chain.to_s}"
  # rule << " -t #{new_resource.table.to_s}"
  rule << " -p #{new_resource.protocol.to_s}" if new_resource.protocol
  rule << " -m #{new_resource.match.to_s}" if new_resource.match
  rule << " -s #{new_resource.source.to_s}" if new_resource.source
  rule << " -d #{new_resource.destination.to_s}" if new_resource.destination
  rule << " -j #{new_resource.jump.to_s}" if new_resource.jump
  rule << " -g #{new_resource.go_to.to_s}" if new_resource.go_to
  rule << " -i #{new_resource.in_interface.to_s}" if new_resource.in_interface
  rule << " -o #{new_resource.out_interface.to_s}" if new_resource.out_interface
  rule << " -f #{new_resource.fragment.to_s}" if new_resource.fragment
  rule << " #{new_resource.extra_options.to_s}" if new_resource.extra_options
  rule << " --comment \"#{new_resource.comment.to_s}\"" if new_resource.comment
  ...
  with_run_context :root do
    edit_resource(:template, new_resource.config_file_full_path) do |new_resource|
      ...
      variables['iptables'][table_name]['chains'][new_resource.chain_target]['rules'] ||= Array.new
